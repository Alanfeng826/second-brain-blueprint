@echo off
rem ---------------------------------------------------------------------------
rem  restart-workbench.bat - restart the detached Workbench dev server.
rem  Use this after editing server-side files (server/*.mjs, the vite plugin):
rem  those are top-level imports and are NOT covered by HMR, so they only take
rem  effect after a full process restart.
...
echo.
echo   Restarting Workbench ...
echo.
"%NODE%" "%~dp0standalone-launcher.mjs" --restart
echo.
echo   Done.
pause
endlocal
